...
async fn test_android_manager_start_device() {
    ...
    let result = android_manager.start_device("Test_Device").await;
    assert!(result.is_ok());
}
...
async fn test_android_manager_stop_device() {
    ...
    // In the actual implementation, we need to resolve the serial number from the device name
    let result = android_manager.stop_device("Test_Device").await;
    // stop_device may try to get the device list internally,
    // so it may result in an error
