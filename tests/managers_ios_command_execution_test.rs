//! Unit tests for managers/ios.rs command execution and parsing code
...
async fn test_ios_manager_performance() {
    ...
    if let Ok(manager) = manager {
        ...
        assert!(
            ...
            "is_available() should complete within 10 seconds, took: {duration:?}"
        );
    ...
    if let Ok(manager) = manager {
        ...
        if let Ok(device_list) = devices {
            for device in device_list {
                ...
                // Test iOS version format
                assert!(
                    device.ios_version.chars().next().unwrap().is_ascii_digit()
                        || device.ios_version.starts_with("iOS"),
                    "iOS version should start with digit or 'iOS'"
                );
            }
        }
    }
