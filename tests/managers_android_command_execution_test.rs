...
async fn test_android_manager_performance() {
    ...
    if let Ok(manager) = manager {
        ...
        assert!(
            ...
            "list_devices() should complete within 10 seconds, took: {duration:?}"
        );
...
async fn test_android_manager_async_behavior() {
    ...
    if let Ok(manager) = manager {
        // Test with timeout
        let result = tokio::time::timeout(Duration::from_secs(15), manager.list_devices()).await;
        ...
        match result {
            ...
            Err(_) => {
                panic!("AndroidManager::list_devices() should complete within 15 seconds");
            }
        }
    }
}
